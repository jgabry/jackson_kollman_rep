...
functions {
  ...
  matrix cov_gen_sq_exp(int T, real eta_sq, real rho_sq, real sigma_sq) {
    matrix[T, T] X ;
    
    // off diagonal elements
    for (t1 in 1:(T-1)) {
      for (t2 in (t1+1):T) {
        X[t1,t2] <- eta_sq * exp(-rho_sq * pow(t1 - t2,2)) ;
        X[t2,t1] <- X[t1,t2] ;
      }
    }
    
    // diagonal elements
    for (t in 1:T) X[t,t] <- eta_sq + sigma_sq ; 
  ...
  
{
  ...
  if (covariance_type == 1) SIGMA <- cov_gen_sq_exp(T, 1.0, 1.0, 0.05) ;
    else 
      if (covariance_type == 2) SIGMA <- cov_matern52(T, jitter) ;

  cholSIGMA <- cholesky_decompose(SIGMA) ;
}
