# same as jk_12_23_2014_6 but with L_Omega ~ lkj_corr_cholesky(2) instead of
# lkj_corr_cholesky(1)
...
parameters {  
  real                          D_0 ;           # intercept
  cholesky_factor_corr[T]       L_Omega ;       # cholesky factor of corr matrix
  vector[T]                     z_Omega ;       # N(0,1) 
  vector<lower=0>[T]            tau_noise ;     # N(0,1) 
  real<lower=0>                 sigma_noise ;   # N(0,1) 
...
transformed parameters {
  ...
  real<lower=0>                 sigma ; # sd for party
  
  beta  <- beta_sd*beta_noise ; 
  delta <- delta_sd*delta_noise ;
  sigma <- cauchy_trans(0, sigma_scale, sigma_noise) ;
...
model {
  # Priors
  D_0 ~ normal(0, 1) ;
  delta_noise ~ normal(0, 1) ; # --> delta ~ normal(delta_mean, delta_sd)
  sigma_noise ~ normal(0, 1) ; # --> sigma ~ half-cauchy(0, sigma_scale)
  ...
  for (j in 1:J) {
    ...
    increment_log_prob(normal_log(col(party, j), mu, sigma)) ;
  }
